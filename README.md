...
一款针对小米 17 Pro Max（骁龙 8 至尊版 SM8750）深度优化的 Magisk/KernelSU 模块，通过智能 CPU 线程绑定策略，为每个应用分配最优核心，实现性能与功耗的完美平衡。
...
- **2+6 全大核架构适配** — 针对骁龙 8 至尊版（SM8750）CPU0-1 超大核 + CPU2-7 性能核的专属调度
...
| CPU 核心 | 编号 | 最高频率 | 定位 |
|---------|------|---------|------|
| 超大核（Prime） | CPU 0-1 | 4.6 GHz | 游戏主线程 / UI渲染 |
| 性能核（Performance） | CPU 2-7 | 3.62 GHz | 应用主线程 / 通用计算 |
...
| 线程类型 | 核心绑定 | 说明 |
|---------|---------|------|
| UnityMain / Unity 渲染 | 6-7 | 性能核大核，满血输出 |
| NativeThread | 3-4 | 性能核中核 |
| Job.Worker | 3-4 | 计算密集型线程 |
| 默认兜底 | 0-5 | 全核可用 |
...
| 线程类型 | 核心绑定 | 说明 |
|---------|---------|------|
| Render Thread / GameThread / MainThread | 6-7 | 性能核大核 |
| *Thread（通用线程） | 6-7 | 优先大核 |
| NativeThread | 3-4 | 性能核中核 |
| 默认兜底 | 0-5 | 全核可用 |
...
- **目标设备**：小米 17 Pro Max（骁龙 8 至尊版 SM8750）
- **目标系统**：Android 14+，Magisk 20.4+ 或 KernelSU
- **CPU 架构**：2+6 全大核（CPU0-1 超大核 4.6GHz + CPU2-7 性能核 3.62GHz）
